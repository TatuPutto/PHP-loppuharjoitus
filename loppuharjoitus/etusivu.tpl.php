<div id="main"> 
	...
	<div id="sivunumero"> <?php 
		...
		for ($i = 1; $i <= $this->sivumaara; $i++) { 
			echo "<a href='?page=etusivu&sivu=".$i."'>".$i."</a> "; 
		} ?>
	</div>
		
    <!--Luodaan taulu, jonka sisällöksi tulostetaan tuotteet taulukon 5 ensimmäistä arvoa-->
	<table>
		<tr>
			<td> <?php 
				$i = 0;
			...
			<td valign="top"> <?php 
				foreach (array_slice($this->tuotteet, 5) as $alkio): ?>
					<div id="etusivu_box"> <?php
						$tuote_id = $alkio["tuote_id"];
						$nimi = $alkio["nimi"];
						$valmistaja = $alkio["valmistaja"]; $lyhyt_kuvaus = $alkio["lyhyt_kuvaus"];  ?>
